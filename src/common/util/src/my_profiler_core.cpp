...
private:
    ...
    void save_to_json() {
        auto out_fn = std::string("profile_core_tracing_log.json");
...
MyProfileCore::MyProfileCore(const std::string& name, const std::vector<std::pair<std::string, std::string>>& args) {
    _name = name;
    _args = args;
    _ts1 = __rdtsc();
}

MyProfileCore::~MyProfileCore() {
    dump_items itm;
    itm.ts2 = __rdtsc();
    itm.ts1 = _ts1;
    itm.name = _name;
