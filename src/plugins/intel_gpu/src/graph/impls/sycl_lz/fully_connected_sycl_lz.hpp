...
struct FullyConnectedImplementationManager : public ImplementationManager {
    ...
    FullyConnectedImplementationManager(shape_types shape_type) : ImplementationManager(impl_types::sycl_lz, shape_type) {}
    std::unique_ptr<primitive_impl> create_impl(const program_node& node, const kernel_impl_params& params) const override;

    bool validate_impl(const program_node& node) const override {
        GPU_DEBUG_LOG << "Not implemented. FullyConnectedImplementationManager::validate_impl" << std::endl;
