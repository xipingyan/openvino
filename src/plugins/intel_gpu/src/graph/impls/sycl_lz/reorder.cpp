...
struct reorder_impl : typed_primitive_sycl_lz_impl<reorder> {
    using parent = typed_primitive_sycl_lz_impl<reorder>;
    using parent::parent;
    // using kernel_selector_t = kernel_selector::reorder_kernel_selector;
    // using kernel_params_t = kernel_selector::reorder_params;

    DECLARE_OBJECT_TYPE_SERIALIZATION(cldnn::sycl_lz::reorder_impl)
    ...
    void load(BinaryInputBuffer& ib) override {
        GPU_DEBUG_LOG << "Not implemented. reorder_impl::load" << std::endl;
        // parent::load(ib);
...
public:
    ...
    static std::unique_ptr<primitive_impl> create(const reorder_node& arg, const kernel_impl_params& impl_param) {
        ...
        if (is_reorder_weights) {
            ...
            GPU_DEBUG_LOG << "reorder_impl::create" << std::endl;
            auto& engine = impl_param.prog->get_engine();
            auto& config = impl_param.prog->get_config();

            return std::make_unique<reorder_impl>(engine, config);
        }
    }
...
std::unique_ptr<primitive_impl> ReorderImplementationManager::create_impl(const program_node& node, const kernel_impl_params& params) const {
    ...
    return sycl_lz::reorder_impl::create(static_cast<const reorder_node&>(node), params);
}
...
std::unique_ptr<primitive_impl> ReorderImplementationManager::create_impl(const kernel_impl_params& params) const {
    ...
    return sycl_lz::reorder_impl::create_reorder_weights(params);
}
