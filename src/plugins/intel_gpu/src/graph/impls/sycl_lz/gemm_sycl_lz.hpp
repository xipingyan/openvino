...
struct GemmSyclLzImplementationManager : public ImplementationManager {
    OV_GPU_PRIMITIVE_IMPL("sycl_lz::gemm")
    GemmSyclLzImplementationManager(shape_types shape_type) : ImplementationManager(impl_types::sycl_lz, shape_type) {}
    std::unique_ptr<primitive_impl> create_impl(const program_node& node, const kernel_impl_params& params) const override;

    bool validate_impl(const program_node& node) const override {
        GPU_DEBUG_LOG << "== GemmSyclLzImplementationManager::validate_impl" << std::endl;
    ...
    in_out_fmts_t query_formats(const program_node& node) const override {
        GPU_DEBUG_LOG << "== GemmSyclLzImplementationManager::query_formats" << std::endl;
        assert(node.is_type<gemm>());
