...
void CL_CALLBACK sycl_lz_event::sycl_lz_event_completion_callback(sycl::event, cl_int, void* me) {
    reinterpret_cast<sycl_lz_event*>(me)->_set = true;
    reinterpret_cast<sycl_lz_event*>(me)->call_handlers();
}

void sycl_lz_event::set_sycl_lz_callback() {
    GPU_DEBUG_LOG << "Not implemented. set_sycl_lz_callback" << std::endl;
    // if (_callback_set)
    //     return;
...
bool sycl_lz_event::get_profiling_info_impl(std::list<instrumentation::profiling_interval>& info) {
    GPU_DEBUG_LOG << "Not implemented. get_profiling_info_impl" << std::endl;
...
void sycl_lz_events::set_impl() {
    wait_impl();
}

bool sycl_lz_events::is_set_impl() {
    GPU_DEBUG_LOG << "Not implemented. wait_impl" << std::endl;
    // try {
    //     if (_last_ocl_event.get() != nullptr) {
...
bool sycl_lz_events::get_profiling_info_impl(std::list<instrumentation::profiling_interval>& info) {
    GPU_DEBUG_LOG << "Not implemented. get_profiling_info_impl" << std::endl;
    ...
    //     sycl_lz_event* be = nullptr;

    //     try {
    //         be = downcast<sycl_lz_event>(_events[i].get());
    //     } catch (const ov::Exception& err) {
    //         GPU_DEBUG_LOG << "WARNING: failed to downcast event to sycl_lz_event - " << err.what() << std::endl;
    //         continue;
    //     }
